...
const handlePlaceBid = async () => {
    ...
    if (!schain) { // request switch network
                    ...
                    <div class="migration-inner">
                        ...
                        <div class="flex flex-col md:flex-row gap-x-4 md:items-center justify-between">
                            <h2 class="text-xl font-otsb uppercase text-white running px-3 py-1 mt-2 inline-block">
                                {{ web3.utils.fromWei(currentRound.prize, 'ether') }} {{
                                    authStore.getCurrentNetwork?.chainSymbol }} /
                                {{ web3.utils.fromWei(currentRound.bidValue, 'ether') }} {{
                                    authStore.getCurrentNetwork?.chainSymbol }}
                            </h2>
                            <button class="submit-button relative z-20 font-otb mt-4 md:mt-0" :loading="submitPlace"
                                :disable="submitPlace" @click="handlePlaceBid">
                        ...
                        <!-- round-num -->
                        <h2 class="text-xl py-2 mt-2 md:text-2xl font-otb uppercase text-main">{{ t('round-info.round-num')
                        }}</h2>
                ...
                <div class="migration-wrapper md:max-w-[60%]">
                    <div class="migration-inner">
                        <h2 class="text-xl pt-4 pb-2 md:text-2xl flex gap-x-8 flex-col md:flex-row">
                            <span class="font-otb uppercase text-main">{{ t('round-info.round-participant') }} - {{
                                currentRound.participants.length }}</span>
                            <button class="flex items-center text-main" @click="copyInviteUrl">
                                <Copy class="text-xl" /><span class="ml-2 text-white font-otr underline text-base">{{
                                    copyText }}</span>
                            </button>
                        </h2>
                        <div v-if="currentRound.participants?.length > 0" class="text-white">
                            <p v-for="(item, index) in currentRound.participants" :key="index"
        ...
        <table class="va-table mt-8 va-table--striped va-table--hoverable w-full">
            ...
            <tbody>
                <tr v-for="(hr, index) in historyRounds.list" :key="index">
                    ...
                    <td class="break-all" v-if="currentRound.number != hr.index">{{ hr.winners?.length > 0 ? hr.winners[0] : '-No Winner-' }}</td>
                    <td class="break-all" v-if="currentRound.number == hr.index">{{ hr.winners?.length > 0 ? hr.winners[0] : '-Running-' }}</td>
