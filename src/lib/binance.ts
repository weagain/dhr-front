...
class Binance implements TradeProxy {
  ...
  public async getMyAllBalances(): Promise<ResponseObj> {
    ...
    try {
      ...
      const response: BinanceAccount = v.data
      return new Promise((resolve, reject) => {
        if (response) {
          const data: any[] = []
  ...
  public async getMarkets(): Promise<ResponseObj> {
    ...
    try {
      ...
      const response: BinanceExchangeInfo = v.data
        ...
        if (response) {
          ...
          for (let i = 0; i < response.symbols.length; i++) {
            const p: TradePair = {
              ...
              amountLowest: response.symbols[i].baseAssetPrecision,
              tradeLowest: response.symbols[i].quoteAssetPrecision,
            }
            data.push(p)
          }
          resolve(this.makeResponse(true, 'success', data))
